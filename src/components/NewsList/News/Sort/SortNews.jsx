...
const SortNews = () => {
  ...
  return (
    <div className='md:flex hidden gap-3 w-full h-fit flex-row-reverse items-center px-2 font-semibold text-sm'>
      ...
      <label onClick={() => {updateParams('CurrentView', 'DESC')}} htmlFor='sortNew1' className='labelSortNews border px-4 py-2 rounded-full cursor-pointer'> پربازدید ترین </label>
      <input type='radio' name='sortNews' id='sortNew2' className='hidden inputSortNews' />
      <label onClick={() => {updateParams('InsertDate', 'DESC')}} htmlFor='sortNew2' className='labelSortNews border px-4 py-2 rounded-full cursor-pointer'> جدید ترین </label>
      <input type='radio' name='sortNews' id='sortNew3' className='hidden inputSortNews' />
      <label onClick={() => {updateParams('UpdateDate', 'DESC')}} htmlFor='sortNew3' className='labelSortNews border px-4 py-2 rounded-full cursor-pointer'> بروز ترین </label>
      <span className='border h-5 border-gray-300'></span>
      <input type='radio' name='sortNews' id='sortNew4' className='hidden' />
      <label onClick={() => {updateParams('', '')}} htmlFor='sortNew4' className='group items-center cursor-pointer hover:bg-red-500 hover:text-white border border-red-500 text-red-500 px-4 py-2 rounded-full flex gap-2 flex-row-reverse'>
