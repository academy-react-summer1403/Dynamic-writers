...
const CoursesView1 = ({isLoaded ,levelName, id, title, describe, dissLikeCount, likeCount, cost, teacherName, date, currentRegistrants, technologyList, tumbImageAddress, statusName}) => {
  ...
  return (
    <Skeleton className='rounded-3xl' isLoaded={isLoaded}>
      <motion.div whileInView={{scale: 1}} whileHover={{ scale: 1.05 }} initial={{y: 1000, scale: 0.3}} animate={{y: 0}} onClick={() => callCourse()} className='rounded-3xl cursor-pointer bg-gray-100 dark:text-white dark:bg-slate-900 iranSans flex flex-col justify-between font-semibold min-w-72' style={{height: '512px', width: '315px'}}>
