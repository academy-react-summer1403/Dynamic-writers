...
import { Calendar02Icon, IdIcon, StartUp01Icon, Time02Icon, Time04Icon, UserStatusIcon } from 'hugeicons-react'
    ...
    <motion.div initial={{scale: 0}} animate={{scale: 1}} transition={{duration: 0.5}} className='iranSans my-2'>
      ...
      <div className='flex flex-col gap-4 w-[289px]'>
            ...
            <div className='relative flex flex-col gap-3'>
                <Select value={timeFrame} placeholder='روز' defaultSelectedKeys='day' className='w-[200px] iranSans' classNames={{trigger: 'bg-white dark:bg-slate-700'}}>
                    ...
                    <SelectItem value="month" key='year' onClick={() => setTimeFrame('year')}> سال </SelectItem>
                </Select>
            </div>
        </div>
      ...
      <div className='mt-[20px] flex justify-start w-full flex-wrap gap-5'>
        {schedule && schedule.length > 0 ? (
          schedule.map((item) => (
            <Card className={`w-[280px] shadow-none dark:bg-slate-700`} key={item.id}>
              <CardBody className={`flex flex-col gap-4 border-[5px] ${item.forming ? 'border-green-500' : 'border-none'}`}>
                <div className='text-right flex items-center gap-2 '>
                    <IdIcon />
                    <span> {item.courseGroupId} </span>
                </div>
                <div className='text-right flex items-center gap-2'>
                    <StartUp01Icon />
                    <span className='flex gap-2'> {jMoment(item.startDate).locale('fa').format('jD jMMMM jYYYY')} <p className='text-sm dark:text-gray-400 text-gray-600'> (شروع) </p> </span>
                </div>
                <div className='text-right flex items-center gap-2'>
                    <Time04Icon />
                ...
                <div className='text-right flex items-center gap-2'>
                    <UserStatusIcon />
                    <span className='flex gap-2'> {`حضور و غیاب شده:`} <div className={`${item.lockToRaise ? 'bg-green-400' : 'bg-red-400' } text-white px-2 py-0.5 rounded-full `}> {item.lockToRaise ? 'بله' : 'خیر'} </div> </span>
                </div>
              </CardBody>
            </Card>
          ))
