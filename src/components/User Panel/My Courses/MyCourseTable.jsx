...
const MyCourseTable = ({ myCourse, isLoading }) => {
  ...
  const filteredCourses = myCourses.filter(item => item.courseTitle !== "آموزش Tailwind css");
                  ...
                  {filteredCourses.map((item, index) => {
                      ...
                      return (
                          <TableRow key={index} className="h-10">
                              ...
                              <TableCell className="text-base font-bold truncate invisible md:visible">
                                  <div className="max-w-40 h-10 truncate leading-8"> {item.courseTitle} </div>
                              </TableCell>
                              <TableCell className="invisible md:visible">
                                  <div className="max-w-56 h-10 truncate leading-8"> {item.fullName.replace('-', ' ')} </div>
                              </TableCell>
                              <TableCell className="invisible md:visible">
                                  <div className="max-w-32 h-10 truncate leading-8"> {(jMoment(item.lastUpdate).locale('fa').format('jD jMMMM jYYYY'))} </div>
                              </TableCell>
                              <TableCell className="text-base font-semibold invisible md:visible">
                              ...
                              <TableCell onClick={() => console.log(item)} className={`invisible md:visible flex whitespace-nowrap gap-2 items-center justify-center py-5 ${item.paymentStatus === "پرداخت نشده" ? "text-red-500 dark:text-white" : "dark:text-white text-green-600"}`}>
                                  {flag !== false && (
                                      <CircularProgressbar
                                          className="size-12"
                                          value={`${percentage !== 100 ? percentage.toFixed(0) : "100"}`}
                                          text={`${percentage !== 100 ? percentage.toFixed(0) : "100"}%`}
                                          styles={buildStyles({
                              ...
                              <TableCell>
                                  {percentage !== 100 && flag !== false && (
                                      <MoneyAdd02Icon
                                          className="size-4 cursor-pointer dark:text-white"
                                          color={`${statusLastPayment[item.courseId] === false ? "red" : "black"}`}
                                          onClick={() => Notif(item.courseId)}
                                      />
                                  )}
                                  {activeModal === item.courseId &&
                                      <MyPaymentModalForUser
                                          isOpen={isOpen}
                                          maxNumber={item.cost - costThisCourse}
