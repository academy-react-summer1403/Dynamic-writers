...
const MyCommentCourseCom = ({                
  ...
  const notifyError = () => { toast.dismiss(), toast.warn(' شما یک بار نظر خود را اعلام کرده اید ') }
  ...
  return (
    <ul className='w-full h-fit flex flex-col gap-4'>
        <div className='w-fit h-[48px] flex gap-2'>
            <img src={profile} className='size-12 rounded-full min-w-12 min-h-12 bg-[#5865F2]' />
        ...
        <div className='flex flex-col gap-2 text-right'>
            <span className='font-[700] text-[#2F2F2F] text-[18px] text-ellipsis whitespace-nowrap dark:text-white'> {title} </span>
            <span className='font-[500] text-[#2F2F2F] text-[16px] text-ellipsis whitespace-nowrap dark:text-white'> {describe} </span>
        </div>
        <div className='flex gap-6 flex-col md:flex-row md:items-center items-start'>
            <div className='flex gap-4'>
                <div className='flex gap-2 flex-row-reverse font-[500] text-[#2F2F2F] text-[16px] items-center dark:text-white'> {likeCount} <ThumbsUpIcon onClick={notifyError2} className='cursor-pointer dark:text-white' /> </div>

                <div className='flex gap-2 flex-row-reverse font-[500] text-[#2F2F2F] text-[16px] items-center dark:text-white'> {dislikeCount} <ThumbsDownIcon onClick={notifyError2} className='cursor-pointer dark:text-white' /> </div>
            </div>
            <div className='flex gap-4 items-center'>
                {!checkAdd && modal && <Button onClick={() => {setCheckAdd(true)}} className='bg-white text-blue-500 border rounded-full border-blue-500 text-base font-semibold dark:bg-slate-700 dark:border-none dark:text-white'> جواب دادن </Button>}
...
export default MyCommentCourseCom
