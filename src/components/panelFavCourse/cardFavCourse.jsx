...
const CardFavCourses = (props) => {
  const { countSearch, rangeIs } = props;
  ...
  const filteredCourses = dataFavCourses
    .filter(el => !countSearch || el.courseTitle.toLowerCase().includes(countSearch.toLowerCase()))
    .filter(el => !rangeIs || el.teacherId === rangeIs);

  useEffect(() => {
    reData();
  }, [slider, countSearch, rangeIs]);
      ...
      <Table aria-label="Example static collection table" dir='rtl'>
        ...
        <TableBody id='list'>
          {filteredCourses.map((el, index) => (
            <TableRow key={index} id='carded' className='hover:bg-slate-100 hover:cursor-pointer rounded-xl'>
              <TableCell>
                <div className='rounded-lg border-1.5 relative bg-slate-500' style={{ width: "104px", height: "72px", top: "3px", overflow: "hidden" }}>
                  <img className='block' style={{ height: "100%" }} src={el.tumbImageAddress} alt="" />
                </div>
              </TableCell>
              ...
              <TableCell>
                <ViewCardFav imaged={el.tumbImageAddress} nameTitle={el.courseTitle} price={el.teacherId} modares={el.teacheName} termNameed={el.termName} />
              </TableCell>
              <TableCell>
