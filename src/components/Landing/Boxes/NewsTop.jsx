...
const NewsTop = ({isLoaded ,title, miniDescribe, currentLikeCount, currentDissLikeCount, addUserFullName, currentView, newsCatregoryName, id, currentImageAddressTumb}) => {
  ...
  return (
    ...
    <motion.div whileInView={{scale: 1}} whileHover={{ scale: 1.02 }} transition={{duration: 0.2}} initial={{y: 1000, scale: 1.05}} animate={{y: 0}} className='rounded-2xl bg-gray-200 dark:bg-slate-900 dark:text-white h-[512px] w-full md:w-[315px] iranSans flex flex-col justify-between font-semibold'>
        ...
        <div className='flex-col flex h-3/6 items-end p-2 overflow-hidden'>
            <h2 className='text-base font-semibold max-w-[200px] overflow-hidden text-ellipsis' dir='rtl'> {title} </h2>
